...
deliq_dt <- read.csv("data/sample_data_intw.csv")
source("~/Desktop/R functions/Cluster analysis and mods/clara_Fun.R")


# Data info 

table(deliq_dt$label)
## 0 == failure, 1 == success, deliq & not 
# 0      1 
# 26162 183431 

## we have duplicate mob numbers, makes snese for multiple transactions 
length(unique(deliq_dt$msisdn))
#1] 186243
dim(deliq_dt)
# [1] 209593     36
...
## daily average spend over 30 & 90 days     
deliq_dt_logs %>%
  ggplot(aes(x = daily_decr90, fill = as.factor(label))) + geom_density()

deliq_dt_logs %>%
  ggplot(aes(x = daily_decr90, fill = as.factor(label))) + geom_density()

## almost perfectly cor, makes sense to use the 90 average spend but could test the pred power of both 
cor(deliq_dt_logs$daily_decr90, deliq_dt_logs$daily_decr30)  





##-- seclct only meaningful cont vars
deliq_dt_logs %>%
